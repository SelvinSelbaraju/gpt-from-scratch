...
def train_test_split(data: torch.Tensor, train_size: float) -> Tuple[torch.Tensor, torch.Tensor]:
    if train_size < 0.0 or train_size > 1.0:
        raise ValueError(f"train_size should be between 0 and 1 but got {train_size}")
